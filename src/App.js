    ...
    <div style={{width:W,height:H,borderRadius:4,overflow:"hidden",position:"relative",
      ...
      <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",
        ...
        <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:9,
          color:"rgba(255,255,255,0.2)",letterSpacing:"0.2em",textTransform:"uppercase",marginTop:4}}>2026 Series 1</span>
...
function Onboarding(props) {
  ...
  var phaseState=useState(initialPhase||(isNewUser?"signup":"login")); var phase=phaseState[0]; var setPhase=phaseState[1];
  useEffect(function(){
    if(initialPhase) setPhase(initialPhase);
    else if(isNewUser) setPhase("signup");
    else setPhase("login");
...
export default function App() {
  ...
  var toppsHeader=(
    <div style={{display:"flex",flexDirection:"column"}}>
      ...
      <div className="topps-header">
        ...
        <button onClick={function(){setPhase("login");setOnboarded(false);setIsNewUser(false);}} className="topps-btn-outline"
        ...
        <div style={{maxWidth:920,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:32,flexWrap:"wrap",position:"relative",zIndex:2}}>
          <div style={{flex:1,minWidth:240}} className="topps-reveal">
            ...
            <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center",marginBottom:28}}>
              <button onClick={function(){setPhase("signup");setOnboarded(false);setIsNewUser(true);}} className="topps-btn-primary" style={{fontSize:16,padding:"14px 40px"}}>
                Claim Free Starter Pack
              </button>
              <button onClick={function(){setPhase("login");setOnboarded(false);setIsNewUser(false);}} className="topps-btn-secondary">
                Sign In
              </button>
            </div>
    ...
    sb(function(db){
      return Promise.all([
        ...
        if(hasCards){
          ...
          // No cards found — could be new user OR RLS blocking the read.
          // NEVER write cards here — only route to onboarding for brand new users.
  ...
  var showOnboarding=authReady&&(!userId||isNewUser)&&((!onboarded&&inventory.length===0)||isNewUser)&&phase!=="landing"&&phase!=="app";
